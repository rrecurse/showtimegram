...
class ShowtimeGram {
    ...
    public function processPost() {
        ...
        if (!empty($this->data['search'])) {
            return $this->filterResult($this->data['search']);
    ...
    private function filterResult($keyword) {
        ...
        try {
            ...
            // # if keyword is ALL indicator, show all
            // # TO-DO - pagination for large results.
            if ($keyword !== '*') {
                $sql = "SELECT * FROM showgrams WHERE caption LIKE '%{$keyword}%' OR filename LIKE '%{$keyword}%'";
            } else {
                $sql = "SELECT * FROM showgrams ORDER BY date DESC LIMIT 10";
            }
